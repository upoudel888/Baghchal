...
export function BaghchalProvider({children}){
    ...
    // pos refers to the node that has been clicked
    // clicker 1 means user and 0 means AI
    // when AI plays skipPostCheck is true and we need not check if it's AIs turn next
    const handleClick = (pos,clicker = 1,skipPostCheck = false) => {  

        if(game.current.isOver()) return;
        //do nothing when user tries to click when it is AI's turn
        ...
        let AITurn = checkAIturn(game.current.getTurnStatus());
        setIsAIturn(AITurn);
        setTimeout(()=>{
        ...
        //couldn't use isAIturn state here because states update asynchronously
        if(skipPostCheck || !AITurn) return;
        ...
        if(!vsPlayer2){
            ...
            if(vsCompGoat && game.current.getTurnStatus()){
                ...
                if(tempArr.length === 2){
                    setTimeout(()=>{
                    handleClick(Number(tempArr[1]),0,true);
                    },215);
                }else{
                    ...
                    setTimeout(()=>{
                    handleClick(Number(tempArr[tempArr.length-1]),0,true);
                    },440);
                }
            }
            //tigers turn
